...
public class AuteurControllerTest {
     ...
     * le controlleur {@link AuteurControlleur} est mocké via Mockito
     */
    @Before
    public void setup() {
    ...
    public void checkGetAllAuteurs() throws Exception {
        mockMvc.perform(get("/auteur")).andExpect(status().isOk())
                ...
                .andExpect(content().bytes(IntegrationTestUtil.convertObjectToJsonBytes(auteurs)));
    ...
    public void checkGetById() throws Exception {
        mockMvc.perform(get("/auteur/1")).andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(content().bytes(IntegrationTestUtil.convertObjectToJsonBytes(auteurs.get(1))));
    ...
    public void checkCreateAuteur() throws Exception {
        mockMvc.perform(
                put("/auteur").contentType(IntegrationTestUtil.APPLICATION_JSON_UTF8).content(
                        IntegrationTestUtil.convertObjectToJsonBytes(auteurTmp))).andExpect(status().isOk())
                .andExpect(content().contentType(IntegrationTestUtil.APPLICATION_JSON_UTF8))
                .andExpect(content().bytes(IntegrationTestUtil.convertObjectToJsonBytes(auteurTmp)));
